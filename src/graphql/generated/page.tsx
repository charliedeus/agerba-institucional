...
export async function getServerPageGetLicitacoes
    (options: Omit<Apollo.QueryOptions<Types.GetLicitacoesQueryVariables>, 'query'>, ctx: ApolloClientContext ){
        const apolloClient = getApolloClient(ctx);
        
        const data = await apolloClient.query<Types.GetLicitacoesQuery>({ ...options, query: Operations.GetLicitacoesDocument });
...
export const useGetLicitacoes = (
  optionsFunc?: (router: NextRouter)=> QueryHookOptions<Types.GetLicitacoesQuery, Types.GetLicitacoesQueryVariables>) => {
  const router = useRouter();
  const options = optionsFunc ? optionsFunc(router) : {};
  return useQuery(Operations.GetLicitacoesDocument, options);
};
export type PageGetLicitacoesComp = React.FC<{data?: Types.GetLicitacoesQuery, error?: Apollo.ApolloError}>;
export const withPageGetLicitacoes = (optionsFunc?: (router: NextRouter)=> QueryHookOptions<Types.GetLicitacoesQuery, Types.GetLicitacoesQueryVariables>) => (WrappedComponent:PageGetLicitacoesComp) : NextPage  => (props) => {
                const router = useRouter()
                const options = optionsFunc ? optionsFunc(router) : {};
                const {data, error } = useQuery(Operations.GetLicitacoesDocument, options)    
                return <WrappedComponent {...props} data={data} error={error} /> ;
                   
            }; 
export const ssrGetLicitacoes = {
      getServerPage: getServerPageGetLicitacoes,
      withPage: withPageGetLicitacoes,
      usePage: useGetLicitacoes,
    }
